"""
Training and evaluation data loaders based on https://github.com/IST-DASLab/gptq ;
taken from https://raw.githubusercontent.com/Vahe1994/AQLM/refs/heads/main/src/datautils.py
"""
import os
import random
...
def get_loaders(
    ...
    if name.lower() == "none":
        ...
        if name.lower() == "wikitext2":
            ...
            print(validation)
            assert torch.all(data_dump[valid_ids] == validation)
            assert torch.all(data_dump[train_ids] == train)
            # exit()

            for t in train:
                for v in validation:
                    assert not torch.allclose(t, v)
            data = torch.zeros((nsamples, 1, seqlen), dtype=torch.long)
            data[valid_ids] = validation
            data[train_ids] = train
            assert torch.all(data_dump == data)
